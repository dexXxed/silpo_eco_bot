...
async def stats(message: types.Message):
    user = Query()
    template = Template("""Вами було накопичено загалом {{sum}} перероблюємих продуктів: 

ПАПЕРУ 📰: {{paper}} арт. {% if x %} - мінус 50 років розкладання на планеті ✅ {% endif %}

ПЛАСТИК ♳: {{plastic}} арт. {% if x %} - врятовано {{plastic // 20}} черепаха 🐢 {% endif %}

МЕТАЛ 🥫: {{metal}} арт. {% if x %} - врятовано {{metal // 10}} єнотик 🦝 {% endif %}

СКЛО 🍾: {{glass}} арт. {% if x %} - врятовано {{glass}} восьминіг 🐙 {% endif %}
